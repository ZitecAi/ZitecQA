...
{
    public static class EnviarArquivoOperacaoNegativo
    {



        public static async Task<TesteNegativoResultado> EnviarArquivoCsvNegativo(IPage page, string fileName, string testName)
        {
        ...
        public static async Task<TesteNegativoResultado> EnviarArquivoCnabNegativo(IPage page, string fileName, string testName)
        {
            string resultado = "❌"; // default como falha
            try
            {
                ...
                await Task.Delay(200);
                await page.Locator("#fileEnviarOperacoes").SetInputFilesAsync(TestesOperacoesOperacoes.Program.Config["Paths:Arquivo"] + fileName);
                await Task.Delay(200);               
                await page.Locator("//button[@id='btnFecharNovoOperacao']").ClickAsync();
                await page.GetByRole(AriaRole.Searchbox, new() { Name = "Pesquisar" }).FillAsync(fileName);
        ...
        public static async Task<TesteNegativoResultado> EnviarArquivoCSVPdfNegativo(IPage Page, string idTeste)
        {
            try
            {
