...
namespace AutomacaoZCustodia.Utils
{
    public class EnviarEmail
    {
        ...
        public static string GerarHtml(List<Pagina> listaPagina, List<Operacoes> listaOperacoes)
        {

            //mudar tabela de erros tirando o perfil dfe usuário master
           ...
           "<h4>Total de Páginas Verificadas: " + totalPaginas + "</h4>";
            ...
            Html += "<h2>Relatório completo do Zitec Custódia</h2>";
            Html += "<table>";
            ...
            Html += "<table>";
            Html += "<tr><th>Tipo operação</th><th>Insert</th><th>Aprovação Consultoria</th><th>Aprovacao Gestora</th><th>Total de erros</th><th>Lista de erros</th></tr>";

            foreach (var operacao in listaOperacoes)
            {
                Html += "<tr>";

                Html += "<td> " + operacao.TipoOperacao + "</td>\n";
                Html += "<td> " + operacao.InsertOperacao + "</td>\n";
                Html += "<td> " + operacao.AprovacaoConsultoria + "</td>\n";
                Html += "<td> " + operacao.AprovacaoGestora + "</td>\n";
                Html += "<td> " + operacao.totalErros + "</td>\n";
                Html += "<td> " + string.Join(", ", operacao.ListaErros1) + "</td>\n";
