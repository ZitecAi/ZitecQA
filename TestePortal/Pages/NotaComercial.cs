                ...
                if (NotaComercial.Status == 200)
                {
                    ...
                    try
                    {
                        ...
                        if (nivelLogado == NivelEnum.Master || nivelLogado == NivelEnum.Gestora || nivelLogado == NivelEnum.Consultoria)
                        {
                            ...
                            await Task.Delay(200);
                            await Page.Locator("#tomador").SelectOptionAsync(new[] { "1" });
                            await Task.Delay(200);
                            ...
                            await Task.Delay(200);
                            await Page.GetByRole(AriaRole.Textbox, new() { Name = "Valor Solicitado:*" }).ClickAsync();
                            await Task.Delay(200);
                            await Page.GetByRole(AriaRole.Textbox, new() { Name = "Taxa de Juros:*" }).ClickAsync();
                            await Page.GetByRole(AriaRole.Spinbutton, new() { Name = "Duração:*" }).ClickAsync();
                            await Task.Delay(200);
                            await Page.GetByRole(AriaRole.Spinbutton, new() { Name = "Duração:*" }).FillAsync("50");
                            await Task.Delay(200);
                            await Page.GetByRole(AriaRole.Spinbutton, new() { Name = "Carência Amortização:*" }).ClickAsync();
                            await Task.Delay(200);
                            await Page.GetByRole(AriaRole.Spinbutton, new() { Name = "Carência Amortização:*" }).FillAsync("10");
                            await Task.Delay(200);
                            ...
                            await Task.Delay(200);
                            await Page.GetByRole(AriaRole.Spinbutton, new() { Name = "Dia de Vencimento:*" }).FillAsync("30");
                            await Task.Delay(200);
                            await Page.GetByRole(AriaRole.Textbox, new() { Name = "/00/0000" }).ClickAsync();
                            await Task.Delay(200);
                            await Page.GetByRole(AriaRole.Textbox, new() { Name = "/00/0000" }).FillAsync("30/04/2025");
                            await Task.Delay(200);
                            await Page.GetByLabel("CORBAN:").ClickAsync();
                            await Task.Delay(200);
                            await Page.Locator("#indexPosFix").SelectOptionAsync(new[] { "CDI" });
                            await Task.Delay(200);
                            await Page.GetByLabel("Percentual Indexador:").ClickAsync();
                            await Task.Delay(200);
                            ...
                            await Page.Locator("#formNotaComercial i").ClickAsync();
                            await Task.Delay(200);
